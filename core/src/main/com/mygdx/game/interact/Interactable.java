	...
	public void renderBottom(PolygonSpriteBatch batch) {
		if (instanceOf.type.collision) {
			...
			batch.draw(
				...
				instanceOf.yPos + (float)instanceOf.type.texStartY / Config.unitHeightInPixels,
				(float) instanceOf.type.texture.getWidth() / Config.unitWidthInPixels,
				(float) instanceOf.type.texture.getHeight() / Config.unitHeightInPixels
			);
			if (currentIngredient != null) {
				batch.draw(
					currentIngredient.texture,
					...
					instanceOf.yPos + (float)(instanceOf.type.texStartY + instanceOf.type.texIngredientStartY) / Config.unitHeightInPixels,
					(float) currentIngredient.texture.getWidth() / Config.unitWidthInPixels,
					(float) currentIngredient.texture.getHeight() / Config.unitHeightInPixels
				);
			}
		}
	}
	...
	public void renderTop(PolygonSpriteBatch batch) {
		if (instanceOf.type.collision) {
			batch.draw(
				instanceOf.type.texture,
				instanceOf.xPos + (float) instanceOf.type.texStartX / Config.unitWidthInPixels,
				max(instanceOf.yPos + instanceOf.type.ySize, instanceOf.yPos
					+ (float) instanceOf.type.texStartY / Config.unitHeightInPixels),
				(float) instanceOf.type.texture.getWidth() / Config.unitWidthInPixels,
				max(0, min((float) instanceOf.type.texture.getHeight() / Config.unitHeightInPixels,
					(float) (instanceOf.type.texture.getHeight() + instanceOf.type.texStartY)
						/ Config.unitHeightInPixels - instanceOf.type.ySize)),
				0,
				0,
				instanceOf.type.texture.getWidth(),
				(int) min(instanceOf.type.texture.getHeight(), instanceOf.type.texture.getHeight()
					- Config.unitHeightInPixels * instanceOf.type.ySize
					+ instanceOf.type.texStartY),
				false,
				false);
			if (currentIngredient != null) {
				batch.draw(
					currentIngredient.texture,
					instanceOf.xPos
						+ (float) (instanceOf.type.texStartX + instanceOf.type.texIngredientStartX)
						/ Config.unitWidthInPixels,
					max(instanceOf.yPos + instanceOf.type.ySize, instanceOf.yPos
						+ (float) (instanceOf.type.texStartY + instanceOf.type.texIngredientStartY)
						/ Config.unitHeightInPixels),
					(float) currentIngredient.texture.getWidth() / Config.unitWidthInPixels,
					max(0, min((float) currentIngredient.texture.getHeight()
							/ Config.unitHeightInPixels,
						(float) (currentIngredient.texture.getHeight() + instanceOf.type.texStartY
							+ instanceOf.type.texIngredientStartY) / Config.unitHeightInPixels
							- instanceOf.type.ySize)),
					0,
					0,
					currentIngredient.texture.getWidth(),
					(int) min(currentIngredient.texture.getHeight(),
						currentIngredient.texture.getHeight() + instanceOf.type.texStartY
							+ instanceOf.type.texIngredientStartY
							- Config.unitHeightInPixels * instanceOf.type.ySize),
					false, false);
			}
		}
